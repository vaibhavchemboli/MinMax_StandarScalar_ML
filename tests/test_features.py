...
class TestFeatures(TestCase):
    ...
    def test_standard_scaler_custom_test(self):
         data = [[1, 2], [3, 4], [5, 6], [7, 8]]
         expected = (data - np.mean(data, axis=0)) / np.std(data, axis=0)
         scaler = StandardScaler()
         scaler.fit(data)
         result = scaler.transform(data)
         assert np.allclose(result, expected), "Custom test failed. Expect {}. Got: {}".format(expected, result)

    def test_standard_scaler_fit_transform(self):
         data = [[116, 172], [729, 112], [224, 256], [64, 128]]
         expected = (data - np.mean(data, axis=0)) / np.std(data, axis=0)
         scaler = StandardScaler()
    ...
    def test_standard_scaler_get_std(self):
        ...
        expected = np.array([0.5, 0.5])
        scaler.fit(data)
        assert (scaler.std_dev == expected).all(), "scaler fit does not return expected mean {}. Got {}".format(expected, scaler.std_dev)
